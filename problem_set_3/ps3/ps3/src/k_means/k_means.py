...
def init_centroids(num_clusters, image):
    ...
    k = num_clusters
    H, W, C = image.shape
    centroids_init = (np.random.rand(k,C) * 255 )//1
...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    for t in range(max_iter):
        ...
        for i in range(n):
            for j in range(k):
                c[j,i] = np.linalg.norm(x[i] - centroids[j]) ** 2
        #print(c)
        c = np.argmin(c, axis=0).reshape(n,) + 1
        #print(c)
        for j in range(1, k):
            c_1 = np.where(c == j,c,0)
            c_1 = np.where(c_1 != j,c_1,1)
            #print(np.sum(c_1))
            numerator = x.T @ c_1
            denominator = np.sum(c_1)
            if denominator == 0:
                centroids[j-1] = (np.random.rand(C) * 255 )//1
            else:
                centroids[j-1] = (numerator / denominator)//1
            #print(centroids[j-1])
        distortion_pre = distortion
        distortion = 0
        for i in range(n):
            distortion += np.linalg.norm(x[i] - centroids[c[i]-1]) ** 2
        if t % print_every == 0:
            print(distortion)
        if distortion - distortion_pre <= 1e-3:
            print('break')
            return centroids
    # *** END YOUR CODE ***
    new_centroids = centroids
...
def update_image(image, centroids):
    ...
    for i in range(n):
        for j in range(k):
            c[j, i] = np.linalg.norm(x[i] - centroids[j]) ** 2
    # print(c)
    c = np.argmin(c, axis=0).reshape(n, )
    for i in range(n):
        x[i] = centroids[c[i]]
    image = np.reshape(x, (H, W, C))
    # *** END YOUR CODE ***
