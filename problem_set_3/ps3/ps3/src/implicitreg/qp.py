...
class QP:
    ...
    def train_SGD(self, X, Y, eta = 8e-2, max_step = 1000, batch_size = 1, 
        ...
        for t in range(max_step):
            ...
            X_batch = X[idx_batch]
            Y_batch = Y[idx_batch]
            #print('Y_ba_shape', Y_batch.shape)
            # *** START CODE HERE ***
            # Compute the gradient of self.theta, self.phi using data X_batch, Y_batch
            # and update self.theta, self.phi
            # grad_theta = np.zeros((X_batch.shape[1],))
            # grad_phi = np.zeros((X_batch.shape[1],))
            # for k in range(batch_size):
            grad_theta, grad_phi = self.gradient(X_batch, Y_batch)
                # #print(grad_theta_new.shape)
    ...
    def gradient(self, X, Y):
        ...
        """
        # *** START CODE HERE ***
        theta_time_x = np.zeros((X.shape[1], X.shape[0]))
        phi_time_x = np.zeros((X.shape[1], X.shape[0]))
        for i in range(X.shape[0]):
            theta_time_x[:, i] = np.multiply(self.theta, X[i])
            phi_time_x[:, i] = np.multiply(self.phi, X[i])
        #print((phi_time_x[:,10] / X[10]) - self.phi)
        power_2 = np.power(self.theta, 2) - np.power(self.phi, 2)
        gradient_theta = theta_time_x @ (X @ power_2 - Y)
        gradient_phi = phi_time_x @ (X @ power_2 - Y)
        return gradient_theta / X.shape[0], -1 * gradient_phi / X.shape[0]
        # *** END CODE HERE ***
