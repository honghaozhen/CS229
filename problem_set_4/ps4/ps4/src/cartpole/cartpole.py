...
def choose_action(state, mdp_data):
    ...
    current_psa = psa[state]
    action_0_psa = current_psa.T[0]
    action_1_psa = current_psa.T[1]
    action_0 = action_0_psa @ value
    action_1 = action_1_psa @ value
    if action_0 > action_1:
        return 0
    elif action_1 > action_0:
        return 1
    else:
...
def update_mdp_transition_counts_reward_counts(mdp_data, state, action, new_state, reward):
    ...
    transition_counts[state, new_state, action] += 1  ### count how many time from si to sj with action n
    if reward == -1:
        reward_counts[new_state, 0] += 1
    reward_counts[new_state, 1] += 1

    mdp_data['transition_counts'] = transition_counts
    mdp_data['reward_counts'] = reward_counts
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    for s in range(psa.shape[0]):
        for s_dot in range(psa.shape[1]):
            for a in range(psa.shape[2]):
                numerator = transition_counts[s, s_dot, a]
                denominator = np.sum(transition_counts[s, :, a])
                if denominator != 0:
                    psa[s, s_dot, a] = numerator / denominator
    for s in range(reward_counts.shape[0]):
        if reward_counts[s, 1] != 0:
            reward[s] = -reward_counts[s,0] / reward_counts[s,1]
    mdp_data['transition_probs'] = psa
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    for i in range(max):
        iter += 1
        pre_value = value
        value = reward + gamma * np.max(value @ psa, axis = 1)
        if np.linalg.norm(value - pre_value) < tolerance:
